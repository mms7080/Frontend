...
export default function RandomBoxPage({ userData }) {
  ...
  const openBox = async () => {
    ...
    const lastOpened = localStorage.getItem(`lastOpenedDate_${userData?.username}`);

    // ✅ 실제 사용 시 주석 해제
     if (lastOpened === today) {
       openModal("오늘은 이미 열었습니다.");
      return;
     }
  ...
  boxSound.currentTime = 0;
    ...
    setTimeout(async () => {
      try {
        ...
        // ✅ 실제 사용 시 주석 해제
       localStorage.setItem(`lastOpenedDate_${userData?.username}`, today);
      } catch (err) {

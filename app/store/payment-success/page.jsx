...
export default function CartPaymentSuccessPage() {
      ...
      try {
        const res = await fetch("http://localhost:9999/api/payments/confirm", {
          ...
          body: JSON.stringify({
            ...
            userId: user?.username || userIdFromQuery,
          }),
        });
  ...
  }, [paymentKey, orderId, amount, user?.id]);
        ...
        {payment && (
          ...
          >
            <div style={{ marginBottom: "20px" }}>
              <p style={{ fontWeight: "bold", fontSize: "18px", margin: 0 }}>{payment.orderName}</p>
              <p style={{ color: "#888", margin: "5px 0" }}>주문번호: {payment.orderId}</p>
            </div>
